...
[[nodiscard]] SceneData build_globe_preview(
    ...
    for (const auto& feature : world.features) {
        ...
        for (const auto& source_ring : feature.rings) {
            const auto curve = view::project_visible_wgs84_linear_ring(
...
[[nodiscard]] SceneData build_globe_verified(
    const source::Result& world,
    const SceneRequest& request,
    const CancelCheck& canceled
) {
    ...
    for (const auto& feature : world.features) {
        ...
        for (const auto& source_ring : feature.rings) {
            ...
            const auto verified =
                view::project_visible_wgs84_linear_polygon_ring_verified(
                    source_ring.geometry,
                    world_to_view,
                    verified_options,
                    scene.globe_radius_m
            ...
            curve_options.geometric_tolerance_m =
    ...
    for (const auto& feature : world.features) {
        ...
        for (const auto& source_ring : feature.rings) {
            ...
            const auto projected =
                projection::project_wgs84_linear_ring_piecewise_verified(
                    source_ring.geometry,
                    options
                );
            if (!projected.ok()) {

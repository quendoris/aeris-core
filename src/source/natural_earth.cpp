...
[[nodiscard]] std::string trim_dbf_spaces(const std::string& value) {
    const auto first = std::find_if(value.begin(), value.end(), [](const char c) {
        return c != ' ';
    });
    const auto last = std::find_if(value.rbegin(), value.rend(), [](const char c) {
...
[[nodiscard]] bool decode_text_property(
    ...
    value = trim_dbf_spaces(record.values[index]);
...
[[nodiscard]] bool decode_positive_int64(
    ...
    if (index >= record.values.size()) return false;
    const std::string text = trim_dbf_spaces(record.values[index]);
...
Result NaturalEarthLand110mAdapter::load(
    ...
    for (const ShapefileRecord& record : parsed.records) {
        ...
        feature.stable_id = std::string(kLandDatasetName) + ":" + manifest.snapshot + ":" + feature.source_id;
...
Result NaturalEarthAdmin0Countries110mAdapter::load(
    ...
    if (request.capability != Capability::admin0) {
        return failure(SourceError::unsupported_capability, "Natural Earth 110m admin0 adapter supplies admin0 countries only");
    }
    if (!request.worldview.empty() && request.worldview != kAdmin0Worldview) {
        return failure(SourceError::unsupported_worldview, "Natural Earth admin0 countries expose only the pinned de-facto worldview");
    }

    const SnapshotManifest& manifest = snapshot.manifest();
    if (manifest.provider != kProviderName || manifest.dataset != kAdmin0DatasetName) {
        return failure(SourceError::malformed_source, "verified snapshot identity does not match Natural Earth 110m admin0 countries");
    }
    if (!request.snapshot.empty() && request.snapshot != manifest.snapshot) {
        return failure(SourceError::unavailable_snapshot, "requested snapshot differs from verified admin0 snapshot");
    }
    ...
    if (!shp_path.has_value() || !dbf_path.has_value() || !cpg_path.has_value() ||
        ...
        return failure(SourceError::provenance_incomplete, "verified Natural Earth admin0 snapshot lacks a required SHP/DBF/CPG/PRJ/VERSION resource");
    }

    const std::string dataset_version = trim_ascii(read_text_file(*version_path));
    if (dataset_version.empty()) {
        return failure(SourceError::provenance_incomplete, "Natural Earth admin0 VERSION.txt is missing or empty");
    }
    if (trim_ascii(read_text_file(*cpg_path)) != "UTF-8") {
        return failure(SourceError::malformed_source, "Natural Earth admin0 DBF encoding marker is not exactly UTF-8");
    }
    if (!recognized_wgs84_prj(read_text_file(*projection_path))) {
        return failure(SourceError::malformed_source, "Natural Earth admin0 .prj is not recognized as WGS84 geographic CRS");
    }

    const ShapefilePolygonResult geometry_table = read_polygon_shapefile(*shp_path);
    if (!geometry_table.ok()) {
        return failure(SourceError::normalization_failed, shapefile_failure_diagnostic(geometry_table));
    }
    const DbfTableResult attributes_table = read_dbf_table(*dbf_path);
    if (!attributes_table.ok()) {
        return failure(SourceError::normalization_failed, dbf_failure_diagnostic(attributes_table));
    }
    if (geometry_table.records.size() != attributes_table.records.size()) {
        return failure(SourceError::malformed_source, "Natural Earth admin0 SHP and DBF physical record counts differ");
    }
    ...
    if (!required_fields) {
        return failure(SourceError::malformed_source, "Natural Earth admin0 DBF lacks the pinned field/type contract");
    }
    ...
    result.provenance.content_sha256 = snapshot.content_sha256();
    result.provenance.retrieved_at_utc = manifest.retrieved_at_utc;
    result.provenance.worldview = kAdmin0Worldview;
    ...
    for (std::size_t row_index = 0U; row_index < geometry_table.records.size(); ++row_index) {
        ...
        if (attribute_record.deleted ||
            ...
            return failure(SourceError::malformed_source, "Natural Earth admin0 SHP/DBF physical record alignment is not exact and active");
        }
        ...
        if (!decode_text_property(attribute_record, name, name_value) ||
            ...
            return failure(SourceError::malformed_source, "Natural Earth admin0 row contains malformed UTF-8 or NE_ID data");
        }
        if (adm0_a3_value.empty() || !seen_adm0_codes.insert(adm0_a3_value).second ||
            !seen_ne_ids.insert(ne_id_value).second) {
            return failure(SourceError::malformed_source, "Natural Earth admin0 identity fields are empty or non-unique");
        }

        Feature feature{};
        feature.source_id = "record:" + std::to_string(geometry_record.record_number);
        feature.stable_id = std::string(kAdmin0DatasetName) + ":ne_id:" + std::to_string(ne_id_value);
    ...
    const SourceError validation = validate_result(*this, request, result);
    if (validation != SourceError::none) {
        return failure(validation, "Natural Earth admin0 adapter output failed common source-adapter validation");
    }
    return result;
}

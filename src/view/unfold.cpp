...
[[nodiscard]] bool append_guide_vertex(
    ...
    const auto globe = orthographic_globe_point(
        longitude,
        beta.value,
        world_to_view,
        radius_m
    ...
    const projection::PlanarResult flat = target_mode == SurfaceMode::sinusoidal
    ...
    line.vertices.push_back({
        {globe.value.x, globe.value.y},
        flat.value,
        globe.value.depth / radius_m,
    });
    return true;
}
...
UnfoldBundle build_unfold_bundle(
    ...
    if (target_mode != SurfaceMode::sinusoidal &&
        target_mode != SurfaceMode::mollweide) {
        bundle.ok = false;
        bundle.diagnostic = "unfold target must be a planar equal-area surface";
        return bundle;
    }
    ...
    globe_request.mode = SurfaceMode::globe;
    ...
    bundle.globe_endpoint = build_scene_geometry(world, globe_request, canceled);
    if (bundle.globe_endpoint.canceled || should_cancel(canceled)) {
        bundle.canceled = true;
        return bundle;
    }
    ...
    bundle.flat_endpoint = build_scene_geometry(world, flat_request, canceled);
    if (bundle.flat_endpoint.canceled || should_cancel(canceled)) {
        bundle.canceled = true;
        return bundle;
    }
    ...
    for (int longitude_deg = -150; longitude_deg <= 150; longitude_deg += 30) {
        if (!append_meridian(
            ...
            if (should_cancel(canceled)) bundle.canceled = true;
                ...
                bundle.ok = false;
                bundle.diagnostic = "unable to build unfold meridian guide";
            }
            return bundle;
        }
    }
    ...
    for (int latitude_deg = -75; latitude_deg <= 75; latitude_deg += 15) {
        if (!append_parallel(
            ...
            if (should_cancel(canceled)) bundle.canceled = true;
                ...
                bundle.ok = false;
                bundle.diagnostic = "unable to build unfold parallel guide";
            }
            return bundle;
        }
    }
    ...
    if (!append_meridian(
        ...
        if (should_cancel(canceled)) bundle.canceled = true;
            ...
            bundle.ok = false;
            bundle.diagnostic = "unable to build unfold seam guides";
        }
        return bundle;
    }

    bundle.diagnostic = target_mode == SurfaceMode::sinusoidal

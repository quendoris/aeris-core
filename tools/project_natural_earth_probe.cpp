...
[[nodiscard]] bool role_matches(
    ...
    const aeris::storage::GeographicRingRole stored) noexcept {
    switch (source) {
        case aeris::source::RingRole::exterior:
            return stored == aeris::storage::GeographicRingRole::exterior;
        case aeris::source::RingRole::interior:
            return stored == aeris::storage::GeographicRingRole::interior;
    }
    return false;
}
...
[[nodiscard]] bool side_matches(
    ...
    const aeris::storage::GeographicInteriorSide stored) noexcept {
    switch (source) {
        case aeris::geometry::RingInteriorSide::unspecified:
            return stored == aeris::storage::GeographicInteriorSide::unspecified;
        case aeris::geometry::RingInteriorSide::left:
            return stored == aeris::storage::GeographicInteriorSide::left;
        case aeris::geometry::RingInteriorSide::right:
            return stored == aeris::storage::GeographicInteriorSide::right;
    }
    return false;
}
...
[[nodiscard]] bool compare_feature(
    ...
    const aeris::storage::SourceFeatureRecord& stored) {
    if (source.stable_id != stored.stable_id || source.source_id != stored.source_id ||
...
[[nodiscard]] bool compare_all_features(
    ...
    const std::vector<aeris::storage::SourceFeatureRecord>& stored) {
    if (reference.features.size() != stored.size()) return false;
    ...
    for (std::size_t index = 0U; index < ordered.size(); ++index) {
        if (!compare_feature(*ordered[index], stored[index])) {
            std::cerr << "feature mismatch after project reopen: " << ordered[index]->stable_id << '\n';
            return false;
        }
    }
    return true;
...
int main(const int argc, char** argv) {
    ...
    if (!reopened.ok()) return fail(14, "project did not reopen after Natural Earth import: " + reopened.status.diagnostic);
    ...
    const aeris::storage::SourceFeatureListResult stored =
        aeris::storage::list_source_features(*reopened.store, kProjectSourceId);
    if (!stored.ok()) return fail(17, "stored Natural Earth geography could not be read: " + stored.status.diagnostic);
    if (!compare_all_features(reference.source, stored.records)) {
        return fail(18, "stored Natural Earth geography differs from canonical adapter output");
    }
    if (!reopened.store->verify_integrity().ok()) {
        return fail(19, "reopened real-world project failed storage integrity verification");
    }

...
int main() {
    ...
    expect(project.ok(), "draft 0.4 project should create");
    if (!project.ok()) {
        std::cerr << project.status.diagnostic << '\n';
        return 1;
    }
    expect(project.store->metadata().format_major == 0 && project.store->metadata().format_minor == 4,
           "project metadata should advertise draft format 0.4");
